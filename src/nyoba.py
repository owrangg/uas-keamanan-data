...
if df_raw is not None:
    ...
    # Prepare Features (Global for simplicity in this script, though ideally scoped)
            ...
            if st.button("Predict"):
                if not user_input:
                    ...
                    if model_type == 'sklearn':
                        # Feature Extraction for single input
                        # Note: We need to transform using the SAME vectorizer fitted on training data
                        # In this simple script, we re-fit vectorizer globally which is not ideal for prod but works for this single-file demo
                        # Ideally, save vectorizer in session_state too.
                        
                        # Re-creating vector input (Hack for this structure)
                        # A better way is to save the vectorizer in session_state during training setup
                        ...
                        probability = -model.decision_function(input_final)[0] # Just a score
